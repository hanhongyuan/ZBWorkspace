package com.sgcc.pms.zbztjc.bbcxgl.sdzzjrqk.bizc;
...
public class SdzzjrqkBizc implements ISdzzjrqkBizc{
	...
	private final Log logger = LogFactory.getLog(SdzzjrqkBizc.class);
	...
	public QueryResultObject sdquery(RequestCondition params) {
		...
		try{
			...
			String querySql =" select linkedprovicedept,LINKEDLINENAME,LINKEDPOLENAME,bzdm,monitoringtypes,DEVICENAME,MANUFACTURER,ATIME,DAYS,SFSS from mw_app.CMSV_ZTJCAPP_SD_DEVINFO_KQV  where 1=1 ";
			...
			logger.info("输电装置接入情况,加载设备列表出错", e);
		} catch(Exception e) {
			logger.info("输电装置接入情况,加载设备列表出错", e);
		}
	...
	private List<DicItems> wrapDictList() {
		...
		String _sql=" select typename,typecode  from MW_APP.cmst_monitoringtype where typecode  like '01%'";
