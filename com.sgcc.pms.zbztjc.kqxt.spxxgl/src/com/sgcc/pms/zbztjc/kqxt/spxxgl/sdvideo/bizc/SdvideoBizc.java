package com.sgcc.pms.zbztjc.kqxt.spxxgl.sdvideo.bizc;
 ...
 * 
 */
public class SdvideoBizc implements ISdvideoBizc{
	...
	private final Log logger = LogFactory.getLog(SdvideoBizc.class);
	@Autowired
	...
	public QueryResultObject query(RequestCondition params) {
		   ...
		   try {
			   ...
			   String querySql =  SqlFileUtil.get("selectSdvideoDevice");
			   ...
			   if(null!=params.getFilter())
			   {
				   ...
				   for(int i = 0; i < qc.length; i++) {
					   ...
					   } else if("linkedline".equals(qc[i].split("=")[0])) {
					   ...
					   } else if("linkedpole".equals(qc[i].split("=")[0])) {
						   querySql += " AND LINKEDPOLENAME like'%" + qc[i].split("=")[1] + "%'";
			   ...
			   querySql += " order by ssdw,xtmc,LINKEDLINENAME ";
