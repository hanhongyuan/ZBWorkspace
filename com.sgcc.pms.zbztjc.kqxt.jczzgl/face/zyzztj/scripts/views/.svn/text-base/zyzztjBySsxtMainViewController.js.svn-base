$ns("zyzztj.views");
$import("zyzztj.views.zyzztjBySsxtMainView");

$import("zyzztj.views.sdzzxxDetailMainViewController")
$import("zyzztj.views.sdzzxxTwoDetailViewController");

$import("zyzztj.views.bdzzxxDetailMainViewController");
$import("zyzztj.views.bdzzxxTwoDetailViewController");

$import("zyzztj.views.zyxtinfoMainViewController");




zyzztj.views.zyzztjBySsxtMainViewController = function()
{
    ...
    me.getView = function()
    {
        if (me.view == null)
        {
            me.view = new zyzztj.views.zyzztjBySsxtMainView({ controller: me });
        }
        return me.view;
    };
    ...
    me._initSdDydjDataGrid = function(){
	    var _dataGrid = me.view.getDataGrid();
	    	_col = _dataGrid.$body.find("td").not("#rownumber,#DWBM,#SDSSJRZZS,#SDSSJRL,#BDSSJRZZS,#BDSSJRL");
	...
	showSdDydjDetail = function(item, colName) {
		 if(colName=="DWMC"){
		zyzzcxtjDetailView = null;
		if (zyzzcxtjDetailView == null) {
			var mvc = new zyzztj.views.zyxtinfoMainViewController();
			zyzzcxtjDetailView = mvc.getView();
		}
		...
		var xtmc = item.item.values.DWMC;						//所属单位
		//zyzzcxtjDetailView.searchBox.editors[0].setValue(xtmc);
		var xtmc_searchBox =  editor.ssxt.value;		//查询框内的所属单位
		var type = null;
		...
		if(null != xtmc){		//如果所属单位为国家电网公司

    		ssxt = item.item.values.DWBM;
    		filter.push("ssxt = " + ssxt);
    		//zyzzcxtjDetailView.searchBox.editors[0].value=ssdw;
    	}
    	filter.push("lx=sd");
		...
		//zyzzcxtjDetailView.searchBox.editors[1].setValue(dydjmc);
		//zyzzcxtjDetailView.searchBox.editors[1].value=dydj;
		//zyzzcxtjDetailView.searchBox.editors[6].value=tyrq;
		//zyzzcxtjDetailView.searchBox.setFieldVisible("tyrq", false)
		//zyzzcxtjDetailView.searchBox.editors[0].setEnabled(false);
		//zyzzcxtjDetailView.searchBox.editors[1].setEnabled(false);
		
		zyzzcxtjDetailView.dataGridSD.setFilter(str.substring(0, str.length - 1));	
  		zyzzcxtjDetailView.dataGridSD.load();
  		
    	zyzzcxtjDetailView.dataGridBD.setFilter(str.substring(0, str.length - 1));
 		zyzzcxtjDetailView.dataGridBD.load();
 		
 		zyzzcxtjDetailView.dataGridcx.setFilter(str.substring(0, str.length - 1));
 		zyzzcxtjDetailView.dataGridcx.load();
		
		me.view.detailWin.setWidth("1000");
    	me.view.detailWin.setHeight("700");
    	me.view.detailWin.setTitle("输电监测装置信息列表");
		me.view.detailWin.setView(zyzzcxtjDetailView);
		me.view.detailWin.showDialog();
	}else if(colName=="SDZZS"){
		zyzzcxtjDetailView = null;
		if (zyzzcxtjDetailView == null) {
			var mvc = new zyzztj.views.sdzzxxDetailMainViewController();
			zyzzcxtjDetailView = mvc.getView();
		}
		var editor = me.view.getSearchBox().editors;
		var jclx = editor.jclx.value;		//监测类型的值
		...
		if("合计" == xtmc){		//如果所属单位为国家电网公司
			
			if(null != xtmc_searchBox){
				filter.push("ssxt = " + xtmc_searchBox);
			}
			//zyzzcxtjDetailView.searchBox.editors[0].value=xtmc_searchBox;
    	}else{
    		ssxt = item.item.values.DWBM;
    		filter.push("ssxt = " + ssxt);
    		//zyzzcxtjDetailView.searchBox.editors[0].value=ssdw;
		...
		//zyzzcxtjDetailView.searchBox.editors[0].setEnabled(false);
		//zyzzcxtjDetailView.searchBox.editors[1].setEnabled(false);
		zyzzcxtjDetailView.dataGrid.setFilter(str.substring(0, str.length - 1));
		zyzzcxtjDetailView.dataGrid.load();
		me.view.detailWin.setWidth("900");
    	me.view.detailWin.setHeight("500");
    	me.view.detailWin.setTitle("输电监测装置信息列表");
		me.view.detailWin.setView(zyzzcxtjDetailView);
		me.view.detailWin.showDialog();
	}else if(colName=="BDZZS"){
		zyzzcxtjDetailView = null;
		if (zyzzcxtjDetailView == null) {
			var mvc = new zyzztj.views.bdzzxxDetailMainViewController();
			zyzzcxtjDetailView = mvc.getView();
		}
		var editor = me.view.getSearchBox().editors;
		var jclx = editor.jclx.value;		//监测类型的值
		...
		if("合计" == xtmc){		//如果所属单位为国家电网公司
			...
			if(null != xtmc_searchBox && '' != xtmc_searchBox.trim()){
				filter.push("ssxt = " + xtmc_searchBox);
			}
			//zyzzcxtjDetailView.searchBox.editors[0].value=xtmc_searchBox;
    	}else{
		...
		zyzzcxtjDetailView = null;
		if (zyzzcxtjDetailView == null) {
			var mvc = new zyzztj.views.sdzzxxTwoDetailViewController();
			zyzzcxtjDetailView = mvc.getView();
		}
		var editor = me.view.getSearchBox().editors;
		var jclx = editor.jclx.value;		//监测类型的值
		...
		me.view.detailWin.setView(zyzzcxtjDetailView);
		me.view.detailWin.showDialog();
	}else if(colName=='BDZS'){
		zyzzcxtjDetailView = null;
		if (zyzzcxtjDetailView == null) {
			var mvc = new zyzztj.views.bdzzxxTwoDetailViewController();
			zyzzcxtjDetailView = mvc.getView();
		}
		var editor = me.view.getSearchBox().editors;
		var jclx = editor.jclx.value;		//监测类型的值
