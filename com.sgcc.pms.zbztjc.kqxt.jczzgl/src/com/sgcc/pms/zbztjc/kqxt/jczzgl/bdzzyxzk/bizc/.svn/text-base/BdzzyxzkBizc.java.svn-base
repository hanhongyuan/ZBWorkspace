package com.sgcc.pms.zbztjc.kqxt.jczzgl.bdzzyxzk.bizc;
...
public class BdzzyxzkBizc implements IBdzzyxzkBizc{
	@Resource
	private IHibernateDao hibernateDao_ztjc;
	
	private final static Log log = LogFactory.getLog(BdzzyxzkBizc.class);
	...
	public QueryResultObject query(RequestCondition queryCondition) {
		...
		String sql = " SELECT OBJ_ID,WSDEPMC,XH as JCXH,linkedprovicedept,DEVICECODE,MONITORINGTYPES,LINKEDSTATIONNAME,LINKEDEQUIPMENTNAME, \n"+
				" LINKEDCAC_DISPLAY,DEVICECATEGORY,DEVICECATEGORY_DISPLAY,DEVICENAME,DEVICEMODEL,MANUFACTURER,RUNDATE,XTMC, \n"+
				" DECODE(sfss,'T','是','F','否','','否',NULL,'否') SFSS FROM MW_APP.CMSV_TRANSFDEVICEF T  \n"+
				" where 1 = 1 and t.WSDEPMC is not null and t.DEVICEVOLTAGE is not null  and MONITORINGTYPES LIKE '02%' "+querySql.toString()+" \n"+
				" order by t.XH ";
		try{
			cols = "OBJ_ID,WSDEPMC,JCXH,LINKEDPROVICEDEPT,DEVICECODE,MONITORINGTYPES,LINKEDSTATIONNAME,LINKEDEQUIPMENTNAME,LINKEDCAC_DISPLAY,DEVICECATEGORY,DEVICECATEGORY_DISPLAY,DEVICENAME,DEVICEMODEL,MANUFACTURER,RUNDATE,XTMC,SFSS";
			...
			e.printStackTrace();
			log.info("执行变电监测装置查询统计的查询初始化DataGrid的查询SQL中发生异常",e);
		}
	...
	public QueryResultObject statByDydj(RequestCondition queryCondition) {
		...
		String sql="with XXX as (select temp.*,\n" +
				...
				"    (SSJRZZS37+SSJRZZS36+SSJRZZS35+ SSJRZZS34+SSJRZZS33+SSJRZZS86+SSJRZZS85+SSJRZZS84+SSJRZZS83+SSJRZZS82) allSSJRZZS,\n" + 
				"    nvl(round(((SSJRZZS37+SSJRZZS36+SSJRZZS35+ SSJRZZS34+SSJRZZS33+SSJRZZS86+SSJRZZS85+SSJRZZS84+SSJRZZS83+SSJRZZS82)/decode(ZZZS37+ZZZS36+ZZZS35+ ZZZS34+ZZZS33+ZZZS86+ZZZS85+ZZZS84+ZZZS83+ZZZS82,0,null,ZZZS37+ZZZS36+ZZZS35+ ZZZS34+ZZZS33+ZZZS86+ZZZS85+ZZZS84+ZZZS83+ZZZS82))*100,2),0)||'%' allSSJRL,\n" + 
				"(ZZZS37+ZZZS36+ZZZS35+ ZZZS34+ZZZS33+ZZZS86+ZZZS85+ZZZS84+ZZZS83+ZZZS82)-(SSJRZZS37+SSJRZZS36+SSJRZZS35+ SSJRZZS34+SSJRZZS33+SSJRZZS86+SSJRZZS85+SSJRZZS84+SSJRZZS83+SSJRZZS82) allWSSJRS\n" + 
				...
				"    ,(select cast(zdypx as varchar2(3)) PMS_XH from MW_APP.cmst_zb_comm_wspz where wsid=X.linkedprovicedept and rownum=1) xh\n" + 
				"   from ( select dydj,\n" + 
				"                  linkedprovicedept,\n" + 
				...
				"  ) a ,MW_APP.cmst_zb_comm_wspz deps where deps.wsid= a.linkedprovicedept(+) " + querySq.toString()+
				...
				"\n" + 
				"\t\t                     from mw_app.CMSV_TransfDEVICEF a\n" + 
				"\t\t                     where  a.linkedprovicedept is not null   and MONITORINGTYPES LIKE '02%'  \n" + querySql.toString()+
	...
	public QueryResultObject statByJclx(RequestCondition queryCondition) {
		...
		try{
			...
			String sql ="with XXX as (select temp.* ,\n" +
					...
					"                      count(distinct devicecode) ssjrzzs\n" + 
					"                     from mw_app.CMSV_TransfDEVICEF a\n" + 
					"                     where  linkedprovicedept is not null   and MONITORINGTYPES LIKE '02%'  "+
					"							\n" +
					"                         AND LINKEDEQUIPMENTDY IS NOT NULL\n" + querySql.toString()+
					"                         "+
					"							and a.devicecode in\n" + 
	...
	public QueryResultObject getDetailList(RequestCondition queryCondition) {
		...
		try{
			cols = "OBJ_ID,LINKEDSTATION,LINKEDSTATIONOBJ,XTJC,XTMC,XTID,LINKEDDEP,SSBDZDYDJ,DEPMC,XH,LINKEDDEPWS,WSDEPMC,WSDEPJC,LINKEDEQUIPMENTDY,DEVICEVOLTAGE," +
					"DYDJGDJB,BZDM,LINKEDEQUIPMENT,LINKEDSTATIONZB,LINKEDSTATIONNAME,LINKEDEQUIPMENTNAME," +
					"LINKEDCAC,LINKEDCAC_DISPLAY,DEVICECATEGORY,DEVICECATEGORYDM,DEVICECODE,DEVICENAME,MONITORINGTYPES,DEVICECATEGORY_DISPLAY,DEVICEMODEL,MANUFACTURER," +
					"RELEASEDATE,RUNDATE,RELEASENUMBER,ISACTIVATED,ISACTIVATED_DISPLAY,EQUIPMENTTYPE," +
					"EQUIPMENTTYPENAME,YXLS,REMARKS,JCXX,SFKQDW,LINKEDPROVICEDEPT,SFSS,TRANSFPHASE,LOOKUP,ISRT";
			result = hibernateDao_ztjc.executeSqlQuery(sql,pageIndex,pageSize);
			result = transToColumns(result, cols);
			count = (List<Number>) (hibernateDao_ztjc.executeSqlQuery("SELECT COUNT(*) FROM (" + sql + ")"));
	...
	public QueryResultObject getFGBDZList(RequestCondition queryCondition) {
		...
		String sql ="select distinct (select n.linkedstation from mw_app.cmst_transfdevice n  \n"+
                       ...
                       "   where t.linkedstation = n.linkedstation  and rownum <= 1) linkedstationname,  max(t3.dydjMC) MC, t1.wsmc PROVINCE_NAME \n"+
