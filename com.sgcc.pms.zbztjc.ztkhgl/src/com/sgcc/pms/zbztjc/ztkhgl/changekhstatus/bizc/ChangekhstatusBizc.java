package com.sgcc.pms.zbztjc.ztkhgl.changekhstatus.bizc;
 ...
 * 
 */
public class ChangekhstatusBizc implements IChangekhstatusBizc{


	private final Log logger = LogFactory.getLog(ChangekhstatusBizc.class);
	@Resource
	...
	/**
	   ...
	   public QueryResultObject loadstatusinfo(RequestCondition params) {
		   ...
		   try {
			  ...
			  // cols = params.getColumns().toString();
			   cols = "OBJ_ID,PROVINCE_NAME,LINKEDDEPTNAME,DEVICENAME,LINKEDDEVICE,TYPENAME,MONITOREDCOMPONENT,CHANGECAUSE,APPLYTIME,APPLICANT,AUDITSTATUS,AUDITVIEW,AUDITTIME";
			   String querySql = " select CAST(OBJ_ID AS VARCHAR2(42)) OBJ_ID,province_name, linkeddeptname, devicename, \n"+
		                " linkeddevice, typename, monitoredcomponent, changecause, \n"+
		                " to_char(applytime,'yyyy-mm-dd hh24:mi:ss') applytime, \n"+
		                " applicant, auditstatus, auditview, \n"+
		                " to_char(audittime,'yyyy-mm-dd hh24:mi:ss') audittime \n"+
		                " from mw_app.cmst_changkhstatus where 1=1";
			   ...
			   loggerSaveBizc.updataUserlog("查询", "状态监测-装置管理-装置考核状态管理", "操作成功");
			   return qro;
		   } catch (Exception e) {
			   logger.info("考核状态列表管理，加载设备列表出错", e);
			   e.printStackTrace();
			   loggerSaveBizc.updataUserlog("查询", "状态监测-装置管理-装置考核状态管理", "操作失败");
		   }
		   return null;
	   }
	   ...
	   public QueryResultObject loadinfo(String id) {
		   ...
		   try {
			  ...
			  // cols = params.getColumns().toString();
			   cols = "OBJ_ID,PROVINCE_NAME,LINKEDDEPTNAME,DEVICENAME,LINKEDDEVICE,TYPENAME,MONITOREDCOMPONENT,COLLECTIONPERIOD,CHANGECAUSE,APPLYTIME,APPLICANT,OLDKHSTATUS,NEWKHSTATUS,AUDITSTATUS,AUDITVIEW,AUDITTIME";
			   String querySql = 
					   " select CAST(OBJ_ID AS VARCHAR2(42)) OBJ_ID,PROVINCE_NAME,LINKEDDEPTNAME,DEVICENAME,LINKEDDEVICE, \n"+
					  " TYPENAME,MONITOREDCOMPONENT,COLLECTIONPERIOD,CHANGECAUSE,APPLYTIME,APPLICANT,OLDKHSTATUS,NEWKHSTATUS,decode(auditstatus,0,1,1,1,2,2) AUDITSTATUS,AUDITVIEW,AUDITTIME \n"+
					  " from mw_app.cmst_changkhstatus where obj_id = '"+id+"'";
	   ...
	   public QueryResultObject saveadui(RequestCondition params) {
		  ...
		  String obj_id = "";
		   ...
		   try {
				   ...
				   AUDITVIEW = filter[1];
				   obj_id = filter[2];
				   querySql = "update mw_app.cmst_changkhstatus set AUDITSTATUS = '"+AUDITSTATUS+"' , AUDITVIEW = '"+AUDITVIEW+"',AUDITTIME = sysdate  where obj_id='"+obj_id+"'";
 ...
 //查询页面下拉框
	   private List<DicItems> wrapDictList() {
		    ...
		    dicts.add(translateFromFile("OLDKHSTATUS", "OLDKHSTATUS"));  
		    dicts.add(translateFromFile("NEWKHSTATUS", "NEWKHSTATUS")); 
	        return dicts;
		}
