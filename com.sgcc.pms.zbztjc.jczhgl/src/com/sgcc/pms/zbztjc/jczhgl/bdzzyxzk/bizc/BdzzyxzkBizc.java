package com.sgcc.pms.zbztjc.jczhgl.bdzzyxzk.bizc;
...
public class BdzzyxzkBizc implements IBdzzyxzkBizc{
	...
	public QueryResultObject query(RequestCondition queryCondition) {
		...
		String sql = " WITH tab AS(SELECT T.*, \n"+
				" (CASE WHEN (SELECT COUNT(1) FROM MW_APP.CMSV_DEVICEUSED_INFO DI WHERE DI.ZZBM = T.DEVICECODE AND DI.SFSS = 'T')>0 THEN 'T' ELSE 'F' END) SFSS FROM (SELECT OBJ_ID,WSDEPMC,XH as JCXH,linkedprovicedept, \n"+
				" DEVICECODE,MONITORINGTYPES,LINKEDSTATIONNAME,LINKEDEQUIPMENTNAME,LINKEDCAC_DISPLAY,DEVICECATEGORY, \n"+
				" DEVICECATEGORY_DISPLAY,DEVICENAME,DEVICEMODEL,MANUFACTURER,RUNDATE,CAST(JCXX AS VARCHAR(42)) JCXX \n"+
				" FROM MW_APP.CMSV_TRANSFDEVICE_XTF t  where 1 = 1 and t.WSDEPMC is not null and t.DEVICEVOLTAGE is not null "+querySql.toString()+" order by t.XH) T  \n"+
				" where t.WSDEPMC NOT LIKE '%电网%' AND t.WSDEPMC NOT LIKE '%分部%' AND MONITORINGTYPES LIKE '02%' AND t.linkedprovicedept IS NOT NULL)  \n"+
				" SELECT tab.*,DECODE(SFSS,'T','是','F','否') IFSS FROM tab where 1 = 1";
