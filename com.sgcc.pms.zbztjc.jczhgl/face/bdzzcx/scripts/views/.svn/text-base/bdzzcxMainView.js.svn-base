	...
	function _initSearchBox() {
		_seSearchBox = new mxpms.datacontrols.AdvancedSearchBox({
			...
			fields : [{
						...
						name : "bdzmc",caption : "变电站名称",displayCheckBox : false,hint:"---请输入---",onchanged:_nameChanged,displayCloseButton : false,editorType : "TextEditor"
					}, {
		...
		_seSearchBox.editors.bdzmc.setCss({width:200});
...
//		$newButtonTd1.append(me.exportBtn.$e);
	...
	function _initDataGrid() {
		var gridEntityContainer = new mx.datacontainers.GridEntityContainer({
					baseUrl : bdzzcx.mappath("~/rest/cmsvtransfdevicext/query"),
					iscID : "-1",
					loadMeta : false,
					primaryKey: "OBJ_ID"
				});
	...
	{
		var bdzmc = _seSearchBox.editors.bdzmc.value;
		
		if(bdzmc!=null&&bdzmc!="")
		{
			if(bdzmc.length>50|| !pattern1.test(sccj)){
				_seSearchBox.editors.bdzmc.reset();
