...
<%@ page import="com.sgcc.pms.zbztjc.spxxgl.bdvideo.bizc.BdvideoBizc" %> 
<%
		...
		//String p_pwd = ApplicationConfigUtil.getString("ZTJC_PASSWORD");
...
<html>
	...
	<script type="text/javascript" src="../showvideo/scripts/jquery.js"></script>
	<script type="text/javascript">
	var uri = "<%=uri%>";
	var p_cabName = "GridControl.cab";
	var p_version = "2.3.0.0";
	var p_HTTPAddr = "10.1.150.251";
	var p_HTTPPort = "7506";
	var p_serverUrl= "<%=p_serverUrl%>";
	var p_pwd= "3dbd957dc5c39ded87286b89e329c405";
	var p_user= "sbdzt";
	var ip="<%=ip%>";
	//alert(p_cabName+"||"+p_version+"||"+p_HTTPAddr+"||"+p_HTTPPort+"||"+p_serverUrl+"||"+p_pwd+"||"+p_user);
	var _height = 500;
	var _width  = 500;
	var _jczzGlobalId = "";
	var state = false;
		...
		//document.getElementById("txtMaxTime").value = new Date().toString("yyyy-MM-dd");
			...
			if(!state){
				...
				if (false){
					...
					document.getElementById("mainvideo").innerHTML ="<object id='player'  classid='CLSID:D71AE918-F986-413A-9BA7-2644172CA228' width='100%"
					+"' height='100%' codebase='assets/OcxPlayer/"+p_cabName+"#version="+p_version+"'><embed width='100%' height='100%' type='application/npGridControl-plugin' id='playerV'  ></object>";
					//+"' height='100%' codebase='../showvideo/resources/"+p_cabName+"#version="+p_version+"'><embed width='100%' height='100%' type='application/npGridControl-plugin' id='playerV'  ></object>";
	...
	function initvideo(){
		...
		//根据选择时间段查询历史视频
		 function execute_query(){
			 document.getElementById("selectVideo").innerHTML="";
			var txtMinTime = document.getElementById("txtMinTime").value;
			var txtMaxTime = document.getElementById("txtMaxTime").value;
...
/* function data_judge(sender)
{
	...
	<table id="zhanshi" class="newmaintable" align="center" cellpadding="0" style="margin-top: 0px;padding-top: 0px" cellspacing="0" >
		...
		<td id="videolist" class="videolist" style="background-color: #c8d0d4;width: 200px;height:400;" align="center">
			<div id="selectlist" class="selectlist" style="margin: 10px;width: 90%;height:90%;">
				<span>输入历史视频日期：</span>
					<br />
					<span>（yyyymmdd）</span>
				<input type="text" class=input  id="txtMinTime"   style="width: 80;height:19px" /><br />
						&nbsp;至&nbsp;<br />
				<input type="text" class=input  id="txtMaxTime"   style="width: 80;height:19px" /><br /><br />
				...
				<span>历史视频列表：</span>
				<br />
				<select id="selectVideo" onchange="showHisVideo()" multiple="multiple" style="width: 85%;height: 55%">
