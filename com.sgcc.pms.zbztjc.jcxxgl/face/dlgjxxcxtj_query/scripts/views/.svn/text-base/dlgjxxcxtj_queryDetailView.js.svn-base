...
dlgjxxcxtj_query.views.dlgjxxcxtj_queryDetailView = function()
{
    ...
    function _initDataForm()
    {
        ...
        _form = new mx.datacontrols.DataForm({
			...
			alias:"dlgjxxcxtj_querydlgjxxcxtj_queryDetailViewDataForm",
    ...
    function _initTab()
    {
    	...
    	function _selection_changing(e){
    		if("monitoringtype" == e.page.name){
    			var dlgjxxcxId = _form.editors["objId"].value;		//获取编号ID
    			//监测类型加载时执行的函数
    		    var client = new mx.rpc.RESTClient();	//初始化RESTClient
    		    var restUrl2 = "~/rest/dlgjxxcxtj_query/queryMonitorRecordById/"+dlgjxxcxId;	//要执行的Controller方法
    		    client.get(dlgjxxcxtj_query.mappath(restUrl2), {"objId":dlgjxxcxId}, function(p) {
	    			if(p.length == 0){
	    				alert("监测记录不存在！")
	    				return;
	    			}
