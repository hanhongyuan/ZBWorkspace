...
$import("dlgjxxcxtj_query.views.MainView");

dlgjxxcxtj_query.views.MainViewController = function()
{
    ...
    me.getView = function()
    {
        if (me.view == null)
        {
            me.view = new dlgjxxcxtj_query.views.MainView({ controller: me });
        }
        return me.view;
    };
  ...
  //查询的方法
    ...
    function _getDetailFromView(objId){
        _detailView = null;
        var mvc = new dlgjxxcxtj_query.views.dlgjxxcxtj_queryDetailViewController();
        ...
        var _tabControl = _detailView.getTabcontrol();
       	...
       	var restUrl = "~/rest/dlgjxxcxtj_query/queryDatailById/"+objId;	//要执行的Controller方法
   		client.get(dlgjxxcxtj_query.mappath(restUrl), {"objId":objId}, function(e) {
   			
			_detailView.getForm().entityContainer.data.setValue("objId",e[0].OBJ_ID) ;
			_detailView.getForm().entityContainer.data.setValue("LINKEDCABLEANDCHANNELNAME",e[0].LINKEDCABLEANDCHANNELNAME) ;
