...
$import("bdgjxxcxtj_stat.views.statByDydjMainView");
$include("~/com.sgcc.pms.zbztjc.util/enum/commonFunction.js");
bdgjxxcxtj_stat.views.statByDydjMainViewController = function()
{
    ...
    me.getView = function()
    {
        if (me.view == null)
        {
            me.view = new bdgjxxcxtj_stat.views.statByDydjMainView({ controller: me });
        }
        return me.view;
    };
   ...
   /* function hidetile(dataGird,dydj){
    ...
    me._initDataGrid = function(){
    	...
    	for(var i=0;i<_col.length;i++){
			if("0" != _col[i].innerText && "" != _col[i].innerText ){
	    		_col[i].setAttribute("class","ywjxLinkBd");
	    	}
		}
    	
    	_dataGrid.$body.find(".ywjxLinkBd").click(function(e) {
    ...
    me._dydjExportBtn_click = function(){
    	...
    	var util = new mxpms.utils.FormDocumentUtil({
    		title:"",
    		view : _dataGrid.$grid,
    		 filename : "变电告警信息按电压等级统计"
    	});
    	 util.exportExcel(); */
    	try {
    		var _dataGrid = me.view.getDataGrid();
        	exportToExcel(_dataGrid,"变电告警信息按电压等级统计");
        	var czlx = "导出";												//此处为操作方式：新增/修改/删除 任选其一
			var transactionType = "状态监测-监测信息查询-变电告警信息按电压等级统计";		//此处为页面的菜单路径信息
