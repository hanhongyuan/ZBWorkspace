...
sdgjxxcxtj_stat.views.statByJclxMainViewController = function()
{
    ...
    me._jclxQueryBtn_click = function(){
    	...
    	var jclx = me.view.getSearchBox().editors.jclx.value;				//电压等级的值
    ...
    me._initDataGrid = function(){
    	...
    	//var _col = _dataGrid.$body.find("#数量").not("#WSDEPMC");
    	 ...
    	 try {
    		 var _dataGrid = me.view.getDataGrid();
    	    	var util = new mxpms.utils.FormDocumentUtil({
    	    		title:"",
    	    		view : _dataGrid.$grid,
    	    		filename : "输电告警信息按监测类型统计"
    	    	});
    	    	 util.exportExcel(); 
    	    	 var czlx = "导出";												//此处为操作方式：新增/修改/删除 任选其一
 				var transactionType = "状态监测-监测信息查询-输电告警信息按监测类型统计";		//此处为页面的菜单路径信息
