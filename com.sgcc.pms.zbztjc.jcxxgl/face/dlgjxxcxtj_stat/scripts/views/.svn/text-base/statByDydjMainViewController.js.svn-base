...
$import("dlgjxxcxtj_stat.views.statByDydjMainView");

dlgjxxcxtj_stat.views.statByDydjMainViewController = function()
{
    ...
    me.getView = function()
    {
        if (me.view == null)
        {
            me.view = new dlgjxxcxtj_stat.views.statByDydjMainView({ controller: me });
        }
        return me.view;
    };
    	 ...
    	 try {
    		 var _dataGrid = me.view.getDataGrid();
    	    	var util = new mxpms.utils.FormDocumentUtil({
    	    		title:"",
    	    		view : _dataGrid.$grid,
    	    		 filename : "电缆告警信息按电压等级统计"
    	    	});
    	    	 util.exportExcel(); 
    	    	 var czlx = "导出";												//此处为操作方式：新增/修改/删除 任选其一
 				var transactionType = "状态监测-监测信息查询-电缆告警信息按电压等级统计";		//此处为页面的菜单路径信息
