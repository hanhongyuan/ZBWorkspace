package com.sgcc.pms.zbztjc.deptip;
...
import com.sgcc.pms.zbztjc.deptip.bizc.IDeptipBizc;
...
@Controller
@RequestMapping("/deptip")
public class deptipController {
	
    @Autowired
    private IDeptipBizc deptipBizc;
...
//    @Transactional
	...
	List<Map> save(@ItemsRequestBody List<Map> list){ 
		try {
			return deptipBizc.saveOrUpdate(list);
		} catch (Exception e) {
	...
	QueryResultObject query(@QueryRequestParam("params") RequestCondition params) {
		return deptipBizc.query(params);
	}
	...
	QueryResultObject query(@PathVariable String id) {
		...
		return deptipBizc.queryById(id);
	}
	 ...
	 */
	public void setManufacturerZbBizc(IDeptipBizc deptipBizc) {
		this.deptipBizc = deptipBizc;
	}
}
