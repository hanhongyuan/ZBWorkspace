package com.sgcc.pms.zbztjc.sdgjxxcxtj_stat.bizc;
...
public class Sdgjxxcxtj_statBizc implements ISdgjxxcxtj_statBizc{
	...
	public QueryResultObject statByDydj(RequestCondition params) {
		...
		try{
			...
			String sql = 
					...
					"SUM(CASE WHEN t.DEVICEVOLTAGEBZDM IN('37','36','35','34','33','32','30','25','81','80','82','83','84','85','86') then t.cnt else 0 END) AS allCNT,\n"+
					...
					"sum(DECODE (t.DEVICEVOLTAGEBZDM,'81',t.cnt,0)) AS CNT81,\n" + 
					...
					"       T.DEVICEVOLTAGEBZDM,T.MONITORINGTYPE,T.LINKEDLINENAME,T.alarmlevelmc,\n" + 
					"       COUNT(*) CNT,sum(DECODE(t.ishandled,'T',1,0)) HCNT,T.ALARMTIME,T.ALARMNUM,\n" + 
					"       SYSDATE statTime,'L' flag\n" + 
					" FROM MW_APP.CMSV_ALARM_SDINFO_XTF T\n" + 
					" WHERE t.WSDEPMC IS NOT NULL\n" + 
					" AND T.ALARMSOURCE = 'E' AND MONITORINGTYPE LIKE '01%' AND ALARMLEVEL IS NOT NULL AND t.WSDEPMC NOT LIKE '%分部%'\n" + 
					" GROUP BY T.LINKEDDEPWS,T.WSDEPMC,T.XH,T.DEVICEVOLTAGEBZDM,T.MONITORINGTYPE,T.LINKEDLINENAME,T.alarmlevelmc,t.ALARMTIME,T.ALARMNUM,to_char(t.ALARMTIME,'yyyy-mm-dd'))";
	...
	public QueryResultObject getDetailList(RequestCondition queryCondition) {
		...
		String cols = "OBJ_ID,LINKEDLINENAME,LINKEDPOLENAME,TYPENAME,JCXX,WSDEPMC,DEVICEVOLTAGE,ALARMMESSAGE,ALARMLEVEL,ALARMTIME,LINKEDDEPWS,DEVICECODE,MONITORINGTYPE";
		String colsTransf = "CAST(OBJ_ID AS VARCHAR2(42)) OBJ_ID,LINKEDLINENAME,LINKEDPOLENAME,TYPENAME,CAST('查看' AS VARCHAR2(10)) JCXX,WSDEPMC,DEVICEVOLTAGE,ALARMMESSAGE,ALARMLEVEL," +
		...
		querySql.append(" FROM mw_app.cmsv_alarm_sdinfo_XTF t ");
		querySql.append(" WHERE T.ALARMSOURCE = 'E' AND MONITORINGTYPE LIKE '01%' ");
