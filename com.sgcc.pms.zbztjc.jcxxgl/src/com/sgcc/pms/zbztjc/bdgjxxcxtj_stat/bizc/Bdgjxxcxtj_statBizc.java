package com.sgcc.pms.zbztjc.bdgjxxcxtj_stat.bizc;
...
import com.sgcc.pms.zbztjc.sdgjxxcxtj_stat.bizc.Sdgjxxcxtj_statBizc;
...
public class Bdgjxxcxtj_statBizc implements IBdgjxxcxtj_statBizc{
	...
	public QueryResultObject statByDydj(RequestCondition params) {
		...
		try{
			...
			String sql = 
					...
					"SUM(CASE WHEN t.DEVICEVOLTAGEBZDM IN('37','36','35','34','33','32','30','25','81','80','82','83','84','85','86') then t.hcnt else 0 END) AS allHCNT,"+
					...
					sql+="(SELECT T.LINKEDDEPWS,T.WSDEPMC,T.XH,\n" +
						"       T.DEVICEVOLTAGEBZDM,T.MONITORINGTYPE,T.LINKEDSTATIONNAME,\n" + 
						"       COUNT(*) CNT,sum(DECODE(t.ishandled,'T',1,0)) HCNT,\n" + 
						"       T.ALARMTIME,T.ALARMLEVEL,T.ALARMNUM,\n" + 
						"       SYSDATE statTime,'T' flag\n" + 
						" FROM MW_APP.CMSV_ALARM_BDINFO_XTF T\n" + 
						" WHERE t.WSDEPMC IS NOT NULL\n" + 
						" AND T.ALARMSOURCE = 'E' AND MONITORINGTYPE LIKE '02%' AND ALARMLEVEL IS NOT NULL AND t.WSDEPMC NOT LIKE '%分部%'\n" + 
						" GROUP BY T.LINKEDDEPWS,T.WSDEPMC,T.XH,t.DEVICEVOLTAGEDM,T.MONITORINGTYPE,T.LINKEDSTATIONNAME,T.ALARMTIME,T.ALARMLEVEL,T.ALARMNUM,to_char(t.ALARMTIME,'yyyy-mm-dd'))";
	...
	public QueryResultObject getDetailList(RequestCondition queryCondition) {
		...
		String cols = "OBJ_ID,LINKEDSTATIONNAME,LINKEDEQUIPMENTNAME,TYPENAME,JCXX,WSDEPMC,DEVICEVOLTAGE,ALARMMESSAGE,ALARMLEVEL,ALARMTIME,LINKEDDEPWS,DEVICECODE,MONITORINGTYPE";
		String colsTransf = "CAST(OBJ_ID AS VARCHAR2(42)) OBJ_ID,LINKEDSTATIONNAME,LINKEDEQUIPMENTNAME,TYPENAME,CAST('查看' AS VARCHAR2(10)) JCXX,WSDEPMC,DEVICEVOLTAGE,ALARMMESSAGE,ALARMLEVEL," +
